...
export async function POST(req: NextRequest, context: { params: Promise<{ strategyToken: string }> }) {
  const { strategyToken } = await context.params
  ...
  const strategy = await prisma.strategy.findUnique({
    where: { webhookToken: strategyToken, isActive: true },
    include: {
      subscribers: {
        where: { isActive: true, userActive: true },
        include: { account: { select: { api_key_enc: true, api_secret_enc: true, delta_account_name: true } } },
      },
    },
  })

  if (!strategy) return NextResponse.json({ error: 'Invalid strategy token' }, { status: 401 })
  ...
  console.log(`Strategy "${strategy.name}" fired: ${success}/${results.length}`)
