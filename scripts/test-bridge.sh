...
R=$(curl -s -X POST "$BASE_URL/api/v1/webhook/BIOUSD" -H "Content-Type: application/json" -d '{"side":"buy","trade":"ENTRY 1 buy"}')
check "Symbol webhook responds" "$R" "success|No active configs|Unknown symbol"
