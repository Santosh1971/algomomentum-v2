// components/ConnectDeltaModal.tsx
// The "Connect Delta Account" modal — styled like Delta Exchange login
// This is the core credential vault UX — 3 steps: Enter keys → Validate → Success
...
export default function ConnectDeltaModal({ tradeConfigId, onSuccess, onClose }: Props) {
  ...
  async function handleConnect() {
    ...
    try {
      const res = await fetch("/api/v1/tradeconfig/verify-keys", {
        ...
        body: JSON.stringify({ api_key: apiKey, api_secret: apiSecret, tradeConfigId }),
      });
        ...
        <div className="px-6 pb-6">
          ...
          {(step === "form" || step === "error") && (
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700">
                🔒 Your credentials are encrypted with AES-256 and never displayed again. We use them only to place orders on your behalf.
              </div>
          ...
          {step === "success" && (
            <div className="py-8 text-center space-y-4">
              ...
              <p className="text-xs text-gray-400">Your credentials are encrypted and saved. They will never be displayed again.</p>
